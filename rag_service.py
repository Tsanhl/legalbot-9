...
def resolve_chroma_persist_directory(persist_directory: Optional[str] = None) -> str:
    """
    Resolve the local ChromaDB directory.

    Priority:
    1) Explicit function argument
    2) CHROMA_PERSIST_DIR env var
    3) ./chroma_db relative to this file
    """
    if persist_directory:
        return os.path.abspath(os.path.expanduser(persist_directory))
...
def _has_local_chroma_db(persist_directory: str) -> bool:
    """Check if a usable local ChromaDB exists."""
    sqlite_path = os.path.join(persist_directory, "chroma.sqlite3")
    return os.path.exists(sqlite_path) and os.path.getsize(sqlite_path) > 0


def maybe_sync_chroma_from_hf(persist_directory: str) -> bool:
    """
    Optionally sync ChromaDB from a Hugging Face dataset before startup.

    Env vars:
    - CHROMA_HF_DATASET_REPO: dataset repo id, e.g. Agnes999/legalbot9
    - CHROMA_HF_AUTO_PULL: 1/0 (default: 1)
    - CHROMA_HF_FORCE_PULL: 1/0 (default: 0)
    - CHROMA_HF_REVISION: optional HF revision/branch/tag
    - HF_TOKEN / HUGGINGFACE_TOKEN / HUGGINGFACEHUB_API_TOKEN: optional auth token
    """
    dataset_repo = (os.getenv("CHROMA_HF_DATASET_REPO") or "").strip()
    if not dataset_repo:
        return False

    if not _env_flag("CHROMA_HF_AUTO_PULL", default=True):
        return False

    force_pull = _env_flag("CHROMA_HF_FORCE_PULL", default=False)
    if not force_pull and _has_local_chroma_db(persist_directory):
        return False

    try:
        from huggingface_hub import snapshot_download
    except Exception as e:
        print(f"⚠️ Hugging Face sync requested but huggingface_hub is unavailable: {e}")
        return False
    ...
    revision = (os.getenv("CHROMA_HF_REVISION") or "").strip() or None

    print(f"⬇️ Syncing ChromaDB from HF dataset: {dataset_repo}")
    try:
        snapshot_download(
        ...
        print("✅ HF ChromaDB sync complete")
        return True
    except Exception as e:
        print(f"⚠️ HF ChromaDB sync failed; using local files if present: {e}")
        return False
...
def get_retrieval_config(query_type: str) -> dict:
    ...
    return QUERY_TYPE_RETRIEVAL_CONFIG.get(query_type, {
...
class RAGService:
    ...
    def __init__(self, persist_directory: str = None, use_upgraded_embeddings: bool = False):
        ...
        """
        persist_directory = resolve_chroma_persist_directory(persist_directory)

        self.persist_directory = persist_directory
        self.use_upgraded_embeddings = use_upgraded_embeddings

        # Optional startup sync for deployment environments (e.g., Render).
        maybe_sync_chroma_from_hf(self.persist_directory)
    ...
    def hybrid_search(
        ...
        for chunk_id in all_chunk_ids:
            ...
            final_score = base_score * category_weight * doc_weight * quality_mult
    ...
    def _multi_hop_retrieval(
        ...
        """
        # Extract entities from top initial results
        top_text = " ".join(r.content for r in initial_results[:10])
        entities = self.extract_legal_entities(top_text)
    ...
    def get_relevant_context(
        ...
        if max_chars <= 0:
            _QUERY_CHARS_BUDGET = {
                ...
                "pb_6500": 105000,
                "pb_7000": 105000,
                "para_improvements": 65000,
                "para_improvements_3k": 75000,
                "para_improvements_5k": 90000,
                "para_improvements_10k": 105000,
                "para_improvements_15k": 105000,
                "advice_mode_c": 75000,
                "sqe1_notes": 105000,
                "sqe2_notes": 105000,
                "sqe_topic": 105000,
        ...
        if query_type and query_type in self._MULTI_HOP_QUERY_TYPES and results:
            try:
...
def get_rag_service() -> RAGService:
    ...
    if _rag_service is None:
        _rag_service = RAGService(persist_directory=resolve_chroma_persist_directory())
    return _rag_service
